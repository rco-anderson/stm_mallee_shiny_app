...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      br(), br(),
      h4("Year selection"),
      actionButton("set_2025", "Edit 25 years"),
      actionButton("set_2100", "Edit 100 years"),
      br(),
...
server <- function(input, output, session) {
  ...
  output$network <- renderVisNetwork({
    ...
    active_edges <- active_edges %>%
      mutate(
        color = ifelse(label != "" & label != "0.00",
                       ifelse(rv$scenario == "degradation", "#000080", "#87CEEB"),
                       "#000000"),
    ...
    Shiny.setInputValue('edge_id', selectedEdgeId, {priority: 'event'});
    ...
    Shiny.setInputValue('edge_id', null, {priority: 'event'});
    this.body.data.edges.forEach(function(edge) {
  ...
  observeEvent(input$submit, {
    ...
    # Combine all edges and check for missing labels (probabilities)
    all_labels_filled <- all(c(
      rv$edges_degradation_2025$label,
      rv$edges_degradation_2100$label,
      rv$edges_regeneration_2025$label,
      rv$edges_regeneration_2100$label
    ) != "")
    
    if (!all_labels_filled) {
      showNotification("Please assign a probability to all transitions before submitting.", type = "error")
      return()
    }
